...
def _verificar_solape_global(
    ...
    *,
    ...
    # Contar solo pares con área de intersección real (no borde compartido).
...
def _exportar_por_huso(gdf: gpd.GeoDataFrame, path: Path) -> None:
    if gdf.empty:
        gdf.to_file(path, driver="GPKG")
        return
    if "utm_zone" not in gdf.columns:
        gdf.to_file(path, driver="GPKG")
        return
    for huso in sorted(gdf["utm_zone"].dropna().unique()):
        sub = gdf[gdf["utm_zone"] == huso].copy()
        epsg = 32718 if int(huso) == 18 else 32719
        out = sub.to_crs(epsg) if sub.crs and sub.crs.to_epsg() != epsg else sub
        suffix = path.stem + (f"_utm{int(huso)}" if huso else "")
        out_path = path.parent / f"{suffix}.gpkg"
...
def ejecutar_seleccion(
    ...
    gdf = _preparar_grilla(grilla_hom, grilla_mix)
    if gdf.crs is None or gdf.crs.to_epsg() != 32719:
        gdf = gdf.to_crs(CRS_PROCESO)
    candidatos_2x2 = grilla_hom.copy()
    if candidatos_2x2.crs is None or candidatos_2x2.crs.to_epsg() != 32719:
        candidatos_2x2 = candidatos_2x2.to_crs(CRS_PROCESO)
    logger.info("Universo nacional: %d rectángulos (%d hom + %d mix)", len(gdf), len(grilla_hom), len(grilla_mix))

    err = candidatos_fuera_bbox(gdf)
    if not err.empty:
        err_path = run_dir / "candidatos_error_probable_c2.gpkg"
        _exportar_por_huso(err, err_path)
    ...
    for eco_id in eco_order:
        ...
        eco_dir = por_eco_dir / f"E{eco_id:02d}"
        eco_dir.mkdir(exist_ok=True)
        _exportar_por_huso(eco_gdf_out, eco_dir / f"seleccion_E{eco_id:02d}.gpkg")
        if not pools_log.empty:
            pools_log.to_csv(eco_dir / f"pools_E{eco_id:02d}.csv", index=False)
    ...
    if not partes:
        ...
        vacio = gpd.GeoDataFrame(columns=["grid_id", "geometry"], crs=gdf.crs)
    ...
    _exportar_por_huso(selected, run_dir / "seleccion_nacional.gpkg")
