...
Uso:
  python visualizar_seg_felzenszwalb_grid.py
  python visualizar_seg_felzenszwalb_grid.py --output-dir /ruta/seg_Felzenszwalb
  python visualizar_seg_felzenszwalb_grid.py --skip-capas   # solo HTML, capas ya exportadas
...
def fila_a_clave(fila: dict) -> str:
    sigma_txt = str(fila["sigma"]).replace(".", "_")
    return f"s{fila['scale']}_sig{sigma_txt}"
...
def descubrir_combinaciones(output_dir: Path, html_dir: Path) -> list[dict]:
    entradas: list[dict] = []
...
function init() {{
  ...
  if (!TIENE_CAPAS) {{
    document.querySelector('.layer-controls').insertAdjacentHTML('beforeend',
      '<span style="color:#b45309;font-size:0.88rem">Capas no exportadas — ejecuta visualizar_seg_felzenszwalb_grid.py sin --skip-capas</span>');
  }}
